...
export default function Modal({ id, title = "modal", isShow = false, handleClose, children }) {
  return (
    <div id={id} tabIndex={isShow ? 1 : -1} aria-hidden={isShow ? "true" : "false"}
      onClick={handleClose}
      className={classNames(
        'overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full transition-colors',
        {
          'flex bg-black/60': isShow,
          'hidden': !isShow,
        }
      )}>
